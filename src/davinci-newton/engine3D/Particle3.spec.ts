...
describe("Particle3", function () {
    xdescribe("updateAngularVelocity", function () {
        xit("dimensionless should accurately update Ω", function () {
            const body = new Particle3(Geometric3.scalar(1 - 0.5 * Math.random()));
            body.M = Geometric3.scalar(1 + Math.random());
            body.L.yz = Math.random();
            body.L.zx = Math.random();
            body.L.xy = Math.random();

            const s = 5 / (2 * body.M.a * 0);
        ...
        describe("dimensioned should accurately update Ω", function () {
            const body = new Particle3(Geometric3.scalar(1 - 0.5 * Math.random(), METER));
