...
describe("PolygonRigidBody2", function () {
    describe("constructor", function () {
        it("constructor", function () {
            const body = new PolygonRigidBody2(unitSquare);
    ...
    describe("Inertia Tensor", function () {
        ...
        it("should depend on width.", function () {
            const body = new PolygonRigidBody2([zero, e1.mulByNumber(2), e1.mulByNumber(2).add(e2), e2]);
            expect(body.I).toBeDefined();
            expect(body.I.dimensions).toBe(1);
            // I = (1/12) * M * (h * h + w * w)
        ...
        it("should depend on height.", function () {
            const body = new PolygonRigidBody2([zero, e1, e1.add(e2.mulByNumber(2)), e2.mulByNumber(2)]);
            expect(body.I).toBeDefined();
            expect(body.I.dimensions).toBe(1);
            // I = (1/12) * M * (h * h + w * w)
            expect(body.I.getElement(0, 0)).toBeCloseTo(5 / 12, 7);
        ...
        it("should have correct units.", function () {
            const body = new PolygonRigidBody2([zero.mul(meter), e1.mul(meter), e1.add(e2.mulByNumber(2)).mul(meter), e2.mulByNumber(2).mul(meter)]);
            body.M = kilogram;
            expect(body.I).toBeDefined();
            expect(body.I.dimensions).toBe(1);
            // I = (1/12) * M * (h * h + w * w)
